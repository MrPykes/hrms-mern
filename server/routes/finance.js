...
router.post('/expenses', roles(['Finance','HR','Admin']), async (req, res) => {
...
router.post('/expenses/:id/approve', roles(['Finance','Admin']), async (req, res) => {
  const exp = await Expense.findById(req.params.id);
  if (!exp) return res.status(404).json({ message: 'Not found' });
  exp.status = 'approved'; exp.approvedBy = req.user._id; await exp.save();
  res.json(exp);
});
...
router.get('/income', roles(['Finance','HR','Admin']), async (req, res) => {
  const list = await Income.find(req.query || {}).sort({ date: -1 }).limit(200);
  res.json(list);
});

router.post('/income/:id/approve', roles(['Finance','Admin']), async (req, res) => {
  const inc = await Income.findById(req.params.id);
  if (!inc) return res.status(404).json({ message: 'Not found' });
  inc.status = 'approved'; inc.approvedBy = req.user._id; await inc.save();
  res.json(inc);
});
